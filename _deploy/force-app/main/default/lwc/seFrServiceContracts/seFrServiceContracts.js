...
export default class SeFrServiceContracts extends NavigationMixin(LightningElement) {
    ...
    wired({ data, error }) {
        ...
        if (data) {
            ...
            this.contracts = data.map((c) => {
                ...
                const deadline = this._deadline(c.endDate);
                return {
                    ...c,
                    slaLabel: this._sla(c.slaHeures),
                    hasSla: c.slaHeures != null,
                    metierStyle: c.metier ? `background:${BADGE_PALETTE[colorIdx]}` : '',
                    hasMetier: !!c.metier,
                    emailHref: c.email ? `mailto:${c.email}` : null,
                    deadlineLabel: deadline.label,
                    deadlineClass: deadline.cssClass,
                    hasDeadline: deadline.label !== ''
                };
            });
    ...
    /**
     * Compute a relative deadline label from the EndDate.
     * Returns { label, cssClass } where cssClass is 'deadline-ok', 'deadline-warning' or 'deadline-expired'.
     */
    _deadline(endDateStr) {
        if (!endDateStr) return { label: '', cssClass: '' };
        ...
        if (diffDays < 0) {
            return {
                label: `${this.dict.expiredSince}`,
                cssClass: 'deadline-expired'
            };
        }
        if (diffDays === 0) {
            return {
                label: this.dict.today,
                cssClass: 'deadline-warning'
            };
        }
        if (diffDays <= 30) {
            return {
                label: `${this.dict.expiresIn} ${diffDays}${this.dict.days}`,
                cssClass: 'deadline-warning'
            };
        }
        return {
            label: `${this.dict.expiresIn} ${diffDays}${this.dict.days}`,
            cssClass: 'deadline-ok'
        };
    }
}
