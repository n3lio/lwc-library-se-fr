  ...
  <div class="docs-grid">
    <aside class="docs-side">
      ...
      <ul>
        ...
        <li><a href="#ai-permsets" {pair_attr('Permsets pour l’IA', 'AI permsets')}>Permsets pour l'IA</a></li>
        <li><a href="#static-resource" {pair_attr('Static Resources', 'Static Resources')}>Static Resources</a></li>
        <li><a href="#troubleshooting" {pair_attr('Galères classiques', 'Common gotchas')}>Galères classiques</a></li>
      </ul>
...
System.debug(res.getStatusCode() + ' ' + res.getBody().abbreviate(500));</pre>
      ...
      <h3 class="subsection" id="ai-permsets" {pair_attr('Permission Sets côté user qui exécute', 'Permission Sets for the running user')}>Permission Sets côté user qui exécute</h3>
      <p {pair_attr('En plus du permset custom <code>Agentforce_API_Access</code> (qui ouvre l’External Credential), assignez ces permsets standards Salesforce au user qui doit pouvoir invoquer les Prompt Templates :', 'On top of the custom <code>Agentforce_API_Access</code> permset (which opens the External Credential), assign these standard Salesforce permsets to the user who needs to run the Prompt Templates:')}>En plus du permset custom <code>Agentforce_API_Access</code> (qui ouvre l'External Credential), assignez ces permsets standards Salesforce au user qui doit pouvoir invoquer les Prompt Templates :</p>
      <ul>
        <li><code>EinsteinGPTPromptTemplateUser</code> — <span {pair_attr('exécution de Prompt Templates depuis l’UI et via Apex.', 'run Prompt Templates from UI and Apex.')}>exécution de Prompt Templates depuis l'UI et via Apex.</span></li>
        <li><code>Agentforce_API_Access</code> — <span {pair_attr('le permset custom du tutoriel ci-dessus, qui mappe l’External Credential principal sur le user.', 'the custom permset from the tutorial above, mapping the External Credential principal to the user.')}>le permset custom du tutoriel ci-dessus, qui mappe l'External Credential principal sur le user.</span></li>
        <li><code>EinsteinGPTSalesSummaries</code> <em>(optionnel)</em> — <span {pair_attr('améliore certains résumés Contact / Opportunity.', 'enhances some Contact / Opportunity summaries.')}>améliore certains résumés Contact / Opportunity.</span></li>
      </ul>
      <p {pair_attr('Pour <code>seFrContactCard</code> en mode AI Summary et <code>seFrPromptLauncher</code>, exemples de Prompt Template API names standards à utiliser dans la prop <code>promptTemplateApiName</code> :', 'For <code>seFrContactCard</code> in AI Summary mode and <code>seFrPromptLauncher</code>, here are example standard Prompt Template API names to set on the <code>promptTemplateApiName</code> prop:')}>Pour <code>seFrContactCard</code> en mode AI Summary et <code>seFrPromptLauncher</code>, exemples de Prompt Template API names standards à utiliser dans la prop <code>promptTemplateApiName</code> :</p>
      <ul>
        <li><code>einstein_gpt__summarizeContact</code> — <span {pair_attr('résumé de Contact (mémo enrichi par AI).', 'Contact summary (AI-enriched memo).')}>résumé de Contact (mémo enrichi par AI).</span></li>
        <li><code>einstein_gpt__summarizeRecord</code> — <span {pair_attr('résumé générique Account / Opportunity / Case.', 'generic summary for Account / Opportunity / Case.')}>résumé générique Account / Opportunity / Case.</span></li>
        <li><code>einstein_gpt__draftEmailFromTemplate</code> — <span {pair_attr('draft d’email depuis un template.', 'email draft from a template.')}>draft d'email depuis un template.</span></li>
      </ul>
      <p class="note" {pair_attr('Les champs lus par le template doivent être <strong>visibles côté user</strong> (FLS Read). Sinon le contexte arrive vide et le template renvoie une réponse vide. Si vous voyez « Réponse du prompt vide » : check FLS sur les champs lus (<code>Title</code>, <code>Department</code>, <code>Email</code>, etc.).', 'Fields read by the template must be <strong>visible to the user</strong> (FLS Read). Otherwise the merge context comes through empty and the template returns an empty response. If you see "Empty prompt response": check FLS on the read fields (<code>Title</code>, <code>Department</code>, <code>Email</code>, etc.).')}>Les champs lus par le template doivent être <strong>visibles côté user</strong> (FLS Read). Sinon le contexte arrive vide et le template renvoie une réponse vide. Si vous voyez « Réponse du prompt vide » : check FLS sur les champs lus (<code>Title</code>, <code>Department</code>, <code>Email</code>, etc.).</p>

      <h2 id="static-resource" {pair_attr('Static Resources requises', 'Required Static Resources')}>Static Resources requises</h2>
      <p {pair_attr('Un seul composant nécessite une Static Resource externe : <code>seFrAgentforceHeader</code>, qui affiche l’image d’Astro (mascotte Salesforce) en background.', 'Only one component needs an external Static Resource: <code>seFrAgentforceHeader</code>, which displays the Astro (Salesforce mascot) image as a background.')}>Un seul composant nécessite une Static Resource externe : <code>seFrAgentforceHeader</code>, qui affiche l'image d'Astro (mascotte Salesforce) en background.</p>
      <p {pair_attr('<strong>Bonne nouvelle</strong> : depuis la version actuelle de la lib, l’image est <strong>auto-bundlée</strong> dans le zip <code>seFrAgentforceHeader.zip</code>. Si vous déployez via le bouton <strong>« Déployer dans mon org »</strong> du site, ou via <code>sf project deploy start -d _deploy/</code>, la Static Resource est créée automatiquement. <strong>Aucune action manuelle nécessaire.</strong>', '<strong>Good news</strong>: as of the current library version, the image is <strong>auto-bundled</strong> in <code>seFrAgentforceHeader.zip</code>. If you deploy via the site’s <strong>"Deploy to my org"</strong> button or via <code>sf project deploy start -d _deploy/</code>, the Static Resource is created automatically. <strong>No manual action needed.</strong>')}><strong>Bonne nouvelle</strong> : depuis la version actuelle de la lib, l'image est <strong>auto-bundlée</strong> dans le zip <code>seFrAgentforceHeader.zip</code>. Si vous déployez via le bouton <strong>« Déployer dans mon org »</strong> du site, ou via <code>sf project deploy start -d _deploy/</code>, la Static Resource est créée automatiquement. <strong>Aucune action manuelle nécessaire.</strong></p>
      <p {pair_attr('Si vous déployez via une méthode alternative (drag-drop dans VS Code par ex.) qui ignorerait le sous-dossier <code>staticresources/</code> du zip, créez la ressource à la main :', 'If you deploy via an alternative method (e.g. drag-drop in VS Code) that ignores the zip’s <code>staticresources/</code> subfolder, create the resource manually:')}>Si vous déployez via une méthode alternative (drag-drop dans VS Code par ex.) qui ignorerait le sous-dossier <code>staticresources/</code> du zip, créez la ressource à la main :</p>
      <ol>
        <li>Setup → Static Resources → <strong>New</strong></li>
        <li>Name : <code>astro_agentforce</code> <span {pair_attr('(exact, case-sensitive — l’import dans le LWC pointe sur ce nom)', '(exact, case-sensitive — the LWC import points to this name)')}>(exact, case-sensitive — l'import dans le LWC pointe sur ce nom)</span></li>
        <li>Cache Control : <code>Public</code></li>
        <li>File : <span {pair_attr('upload une PNG d’Astro. Vous trouverez l’image dans le zip <code>seFrAgentforceHeader.zip</code> (sous <code>staticresources/astro_agentforce.png</code>) ou dans le repo source sous <code>Original Assets/IMAGES/astro_agentforce.png</code>.', 'upload a PNG of Astro. The image is in <code>seFrAgentforceHeader.zip</code> (under <code>staticresources/astro_agentforce.png</code>) or in the repo source under <code>Original Assets/IMAGES/astro_agentforce.png</code>.')}>upload une PNG d'Astro. Vous trouverez l'image dans le zip <code>seFrAgentforceHeader.zip</code> (sous <code>staticresources/astro_agentforce.png</code>) ou dans le repo source sous <code>Original Assets/IMAGES/astro_agentforce.png</code>.</span></li>
        <li>Save</li>
      </ol>

      <h2 id="troubleshooting" {pair_attr('Galères classiques (et résolutions)', 'Common gotchas (and fixes)')}>Galères classiques (et résolutions)</h2>
      <p {pair_attr('Les soucis qui reviennent souvent quand on configure la lib pour la première fois sur une org. Au moindre doute, retombez ici.', 'Issues that come up when setting up the library for the first time on an org. When in doubt, scroll back to this list.')}>Les soucis qui reviennent souvent quand on configure la lib pour la première fois sur une org. Au moindre doute, retombez ici.</p>
      <ul>
        <li><strong>« We couldn’t access the credential(s) Agentforce_External_Auth »</strong> → <span {pair_attr('le permset <code>Agentforce_API_Access</code> n’est pas assigné au user qui exécute. Setup → Permission Sets → Manage Assignments → Add.', 'the <code>Agentforce_API_Access</code> permset isn’t assigned to the running user. Setup → Permission Sets → Manage Assignments → Add.')}>le permset <code>Agentforce_API_Access</code> n'est pas assigné au user qui exécute. Setup → Permission Sets → Manage Assignments → Add.</span></li>
        <li><strong {pair_attr('« Réponse du prompt vide »', '"Empty prompt response"')}>« Réponse du prompt vide »</strong> → <span {pair_attr('soit le permset <code>EinsteinGPTPromptTemplateUser</code> manque, soit la FLS bloque les champs lus par le template (<code>Title</code>, <code>Department</code>, etc.).', 'either <code>EinsteinGPTPromptTemplateUser</code> is missing, or FLS is blocking fields read by the template (<code>Title</code>, <code>Department</code>, etc.).')}>soit le permset <code>EinsteinGPTPromptTemplateUser</code> manque, soit la FLS bloque les champs lus par le template (<code>Title</code>, <code>Department</code>, etc.).</span></li>
        <li><strong {pair_attr('« Invalid reference astro_agentforce »', '"Invalid reference astro_agentforce"')}>« Invalid reference astro_agentforce »</strong> → <span {pair_attr('la Static Resource <code>astro_agentforce</code> manque dans l’org cible. Si vous déployez via le bouton du site, c’est auto. Sinon voir la section <a href="#static-resource">Static Resources</a> ci-dessus.', 'the <code>astro_agentforce</code> Static Resource is missing in the target org. If you deploy via the site button, it’s automatic. Otherwise see <a href="#static-resource">Static Resources</a> above.')}>la Static Resource <code>astro_agentforce</code> manque dans l'org cible. Si vous déployez via le bouton du site, c'est auto. Sinon voir la section <a href="#static-resource">Static Resources</a> ci-dessus.</span></li>
        <li><strong {pair_attr('Déploiement « 5/6 réussis » mais 0 dans l’org', 'Deploy says "5/6 succeeded" but 0 components in the org')}>Déploiement « 5/6 réussis » mais 0 dans l'org</strong> → <span {pair_attr('cas d’un déploiement où Salesforce a fait un rollback total à cause d’une erreur sur 1 composant. Le bouton du site est passé en mode <em>best-effort partial deploy</em> (les OK arrivent même si 1 KO), donc ce souci ne devrait plus se produire.', 'this happens when Salesforce rolled back the whole transaction because of one failing component. The site button now uses <em>best-effort partial deploy</em> (successes still land even if one fails), so this should no longer occur.')}>cas d'un déploiement où Salesforce a fait un rollback total à cause d'une erreur sur 1 composant. Le bouton du site est passé en mode <em>best-effort partial deploy</em> (les OK arrivent même si 1 KO), donc ce souci ne devrait plus se produire.</span></li>
        <li><strong {pair_attr('Le bot Showcase atterrit sur la page de login (frontdoor.jsp refusé)', 'The Showcase bot lands on the login page (frontdoor.jsp rejected)')}>Le bot Showcase atterrit sur la page de login (frontdoor.jsp refusé)</strong> → <span {pair_attr('Salesforce ne mint que des tokens <code>api</code>-only via <code>client_credentials</code>. Pour ouvrir une session UI il faut <strong>JWT Bearer Flow</strong> (cert RSA + signature) qui produit un token avec scope <code>web</code> accepté par frontdoor.', 'Salesforce only mints <code>api</code>-only tokens via <code>client_credentials</code>. To open a UI session you need <strong>JWT Bearer Flow</strong> (RSA cert + signature) which produces a token with the <code>web</code> scope that frontdoor accepts.')}>Salesforce ne mint que des tokens <code>api</code>-only via <code>client_credentials</code>. Pour ouvrir une session UI il faut <strong>JWT Bearer Flow</strong> (cert RSA + signature) qui produit un token avec scope <code>web</code> accepté par frontdoor.</span></li>
      </ul>
